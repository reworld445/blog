...
import { getBlogList, delBloglog } from "@/api";
export default {
  name: "bloglist",
  components: {
    BlogEdit,
  ...
  methods: {
    ...
    async getBlogList(page) {
      const res = await getBlogList({ type: "admin", page: page });
    ...
    async handDelete(index, item) {
      const res = await delBloglog({ id: item._id });
      // console.log(res)
