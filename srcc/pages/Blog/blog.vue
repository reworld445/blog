    ...
    <div class="studylog_warp">
      <el-row>
        ...
        <el-col :xs="0" :sm="8" :md="8" :lg="8" :xl="8">
          <!-- <study-category :lsit="lsit"></study-category> -->
          <blog-category v-on:update="updatelist"></blog-category>
        </el-col>
      </el-row>
    </div>
  </div>
...
export default {
  ...
  computed: {
    noMore() {
      return this.list.length >= this.scrollbarTotal;
    },
    disabled() {
  ...
  methods: {
    ...
    async getBlogList(page) {
      let res = await getBlogList({ page: page });
  ...
  mounted() {
    document.addEventListener("scroll", async () => {
      if (
        ...
        this.page++;
        // this.loading = true;
        await this.getBlogList(this.page);
        this.$nextTick(() => {
          this.moreFlag = true;
          // this.loading = false;
        });
